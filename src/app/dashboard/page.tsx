    ...
    <div className="min-h-screen bg-background pt-24 pb-20">
      ...
      <div className="container px-4 mx-auto">
        ...
        <Tabs defaultValue="listings" className="space-y-8">
          <TabsList className="bg-white/50 backdrop-blur-md border p-1 rounded-2xl h-14 gap-2 flex-wrap sm:flex-nowrap">
            <TabsTrigger value="listings" className="flex-1 rounded-xl px-4 sm:px-8 h-12 data-[state=active]:bg-primary data-[state=active]:text-white font-bold shadow-sm transition-all">Listings</TabsTrigger>
            <TabsTrigger value="bookings" className="flex-1 rounded-xl px-4 sm:px-8 h-12 data-[state=active]:bg-primary data-[state=active]:text-white font-bold shadow-sm transition-all">Bookings</TabsTrigger>
            <TabsTrigger value="profile" className="flex-1 rounded-xl px-4 sm:px-8 h-12 data-[state=active]:bg-primary data-[state=active]:text-white font-bold shadow-sm transition-all">Profile</TabsTrigger>
          </TabsList>

          <TabsContent value="listings" className="space-y-6">
            {allListings.length > 0 ? allListings.map((listing) => (
              <Card key={listing.id} className="border-none shadow-md overflow-hidden rounded-[32px] bg-white">
                <CardContent className="p-6 flex flex-col md:flex-row gap-6 items-center">
                  ...
                  <div className="flex-1 text-center md:text-left min-w-0">
                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                      ...
                      <Badge variant={listing.status === 'active' ? 'default' : 'secondary'} className="rounded-full px-3">{listing.status}</Badge>
                    </div>
                    <p className="text-primary font-bold text-lg">{listing.currency} {listing.price}</p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <Link href={`/listings/edit/${listing.id}?path=${listing._collectionPath}`}>
                      <Button variant="outline" className="rounded-xl px-6 border-2 font-bold hover:bg-primary hover:text-white hover:border-primary transition-all">
                        <Edit3 size={16} className="mr-2" /> Edit
                      </Button>
                    </Link>
                    <Button variant="ghost" className="rounded-xl text-destructive hover:bg-destructive/10 transition-colors" onClick={() => handleDeleteListing(listing.id, listing._collectionPath)}>
                      <Trash2 size={18} />
                    </Button>
                  </div>
                </CardContent>
            ...
            <section>
              ...
              <div className="grid gap-4">
                {(receivedBookings || []).length > 0 ? receivedBookings?.map((booking) => (
                  <Card key={booking.id} className="border-none shadow-md rounded-[32px] bg-white">
                    <CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
                      ...
                      <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                        {booking.status === 'pending' ? (
                          <>
                            <Button variant="outline" className="rounded-xl text-green-600 border-green-200 border-2 font-bold flex-1 sm:flex-none hover:bg-green-600 hover:text-white hover:border-green-600 transition-all" onClick={() => handleBookingStatus(booking.id, 'accepted')}>
                              <CheckCircle size={16} className="mr-2" /> Accept
                            </Button>
                            <Button variant="ghost" className="rounded-xl text-destructive hover:bg-destructive/10" onClick={() => handleBookingStatus(booking.id, 'rejected')}>
                              <XCircle size={18} />
                            </Button>
                          </>
                        ) : (
                          <Badge className="rounded-full capitalize px-6 py-1.5 font-bold shadow-sm">{booking.status}</Badge>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                )) : (
                  <div className="text-center py-10 bg-white/50 rounded-[32px] border border-dashed border-gray-200">
          ...
          <TabsContent value="profile" className="grid md:grid-cols-3 gap-8">
            <Card className="col-span-1 rounded-[40px] border-none shadow-md bg-white overflow-hidden">
              <div className="h-32 bg-primary relative">
                <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-4 border-white overflow-hidden bg-gray-200 flex items-center justify-center shadow-lg">
              ...
              <CardContent className="pt-16 pb-10 text-center">
                <h4 className="font-headline font-bold text-2xl mb-1">{user?.displayName || 'Sports Provider'}</h4>
                <p className="text-sm text-muted-foreground mb-6">{user?.email}</p>
                <Button variant="outline" className="w-full rounded-xl border-2 font-bold transition-all" onClick={() => fileInputRef.current?.click()}>
            ...
            <Card className="col-span-2 rounded-[40px] border-none shadow-md bg-white p-10">
              <div className="grid gap-8">
                <div className="grid gap-3">
                  <Label className="font-bold text-sm uppercase tracking-widest text-gray-500">Display Name</Label>
                  <Input value={profileData.displayName} onChange={e => setProfileData(p => ({...p, displayName: e.target.value}))} placeholder={user?.displayName || 'Your Name'} className="rounded-xl h-14 bg-background border-none focus-visible:ring-2" />
                </div>
                <div className="grid gap-3">
                  <Label className="font-bold text-sm uppercase tracking-widest text-gray-500">Bio</Label>
                  <textarea className="w-full rounded-xl border-none bg-background p-4 min-h-[150px] focus:ring-2 focus:ring-primary outline-none transition-all" value={profileData.bio} onChange={e => setProfileData(p => ({...p, bio: e.target.value}))} placeholder="Tell the community about your sports expertise..." />
                </div>
                <Button className="rounded-xl h-14 font-bold text-lg shadow-lg shadow-primary/20" onClick={handleSaveProfile}>Save Profile Changes</Button>
              </div>
            </Card>
          </TabsContent>
        </Tabs>
